def format_linter_error(error: dict) -> dict:
    return [{"errors": [{
        ...
        "source": "flake8"}if len(error[key]) else [] for i in range(len(error[key]))],
        "path": key,
...
def format_single_linter_file(file_path: str, errors: list) -> dict:
    return [{"errors": [{
        ...
        "path": key,
        "status": "failed" if len(errors[key]) else "passed"} for key in errors if file_path == key][0]
...
def format_linter_report(linter_report: dict) -> list:
    return [{"errors": [{
        ...
        "source": "flake8"}
        if len(linter_report[key]) else []for i in range(len(linter_report[key]))],
        "path": key,
        "status": "failed" if len(linter_report[key]) else "passed"}for key in linter_report]
